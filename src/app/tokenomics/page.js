"use client";

import HoverScroll from "@/components/HoverScroll";
import TypingOnLoad from "@/components/TypingOnLoad";
import ScrollingNumber from "@/components/scrollingNumbers";
import { useState } from "react";
import { Button, Grid2, Box, Typography } from "@mui/material";
import { Surface, BarChart, Bar, Area, Layer, PieChart, Pie, Cell, Rectangle, AreaChart, ResponsiveContainer, LineChart, CartesianGrid, XAxis, YAxis, Tooltip, Legend, Line } from "recharts"
import "../fadeinAnim.css"
import { FlightTakeoffSharp } from "@mui/icons-material";
import Footer from "@/components/Footer";
import HeaderNoFadeIn from "@/components/HeaderNoFadeIn"


export default function TokenomicsPage() {
    const trasuriesData = [{
        treasury: "", allocation: "", percentage: 50
    }]

    const [connectWallet, setConnectWallet] = useState(false)
    const [selectionNum, setSelectionNum] = useState(null) // this is bad code, sorry......
  

    const treasuryList = [
        {treasury: "SOL 100M LPS ( LOCKED & BURNS FOR UNCOMMITTED LPS)	", allocation: "50,000,000", percentage: 5},
        {treasury: "INDEX UNLOCKED TREASURY ( NAV )", allocation: "50,000,000", percentage: 20},
        {treasury: "INDEX UNLOCKED TREASURY ( COMMUNITY VOTING )", allocation: "50,000,000", percentage: 50},
        {treasury: "INDEX UNLOCKED TREASURY ( BACKUP )", allocation: "50,000,000", percentage: 36},
        {treasury: "ECOSYSTEM TREASURY", allocation: "50,000,000", percentage: 67},
        {treasury: "OSOL VENTURES", allocation: "50,000,000", percentage: 32},
        {treasury: "MARKET MAKERS", allocation: "50,000,000", percentage: 12},
        {treasury: "OSOL CHARITY", allocation: "50,000,000", percentage: 56},
    ]

    const dataList = [
        {title: "SOL 100M LPs", content: "The allocations are distributed among over 50 LPs, with each LP contributing over $2M to the DAO Fund smart wallet. These projects invest in us, just as we invest in them through the index."},
        {title: "Index Unlocked Treasury", content: "Are wallets that hold the OSOL coins for the purpose of contributing its value inside the main fund smart wallet to grow it, a proportion of the allocation trading decisions should be governed by community voting, and a proportion should be automated by missO and the market makers. 1. 5% will be directly added to the smart wallet to sustain a high fund NAV. 2. 5% will be allocated for community-driven requests for coin purchases, based on collective voting. 3. 10% will be prepared for allocation to one of the aforementioned wallets."},
        {title: "OSOL Ventures", content: "OSOL Ventures introduces a revolutionary investment model where artificial intelligence meets community-driven venture capital. As a decentralized investment vehicle powered by MissO (the world's first AI CEO) and governed by OSOL token holders, OSOL Ventures represents a paradigm shift in how venture capital operates in the Web3 space."},
        {title: "Ecosystem & OBOT Treasury", content: ""},
        {title: "OSOL CHARITY", content: "Are wallets that hold the OSOL coins for the purpose of contributing its value inside the main fund smart wallet to grow it, a proportion of the allocation trading decisions should be governed by community voting, and a proportion should be automated by missO and the market makers. 1. 5% will be directly added to the smart wallet to sustain a high fund NAV. 2. 5% will be allocated for community-driven requests for coin purchases, based on collective voting. 3. 10% will be prepared for allocation to one of the aforementioned wallets."},

    ]    

    const makePercentage = (num) => {
        let string = []
        for (let i = 0; i <= 50; i++) {
            string.push(<b style={{color: (i <= Math.floor(num/2)) ? "white" : "rgba(20, 20, 20, 1)"}}>|</b>)
        } 
        
        return(string)
    }   

    return (
        <>

            { connectWallet ?
                    <div style={{position: "fixed", display: "flex", justifyContent: "center", alignItems: "center", backgroundColor: "red", width: "100%", height: "100%", zIndex: 10000000, backgroundColor: "rgba(0, 0, 0, 0.2)", backdropFilter: "blur(2px)"}}>
                        <div style={{width: 360, height: 500, backgroundColor: "rgb(8, 8, 8)"}}>
                        <Grid2 style={{height: "100%", width: "100%"}} container>
                            <Grid2 style={{border: "1px solid rgba(255, 255, 255, 0.2)", height: "15%"}} size={12} item>
                            <div style={{border: "1px solid rgba(255, 255, 255, 0.1)", height: "100%", color: "white", display: "flex", justifyContent: "center", alignItems: "center", fontWeight: "300", fontSize: 15}}>
                                CONNECT SOLANA WALLET TO CONTINUE
                            </div>
                            </Grid2>
                            <Grid2 style={{border: "1px solid rgba(255, 255, 255, 0.2)", display: "block", height: "85%"}} size={12} item>
                            <div onClick={() => {setSelectionNum(1)}} className='rectangles2' style={{width: "80%", border: "1px solid rgba(255, 255, 255, 0.1)", display: "flex", alignItems: "center", alignContent: "center", marginLeft: "50%", transform: "translateX(-50%)", height: 50, display: "flex", backgroundColor: "rgb(10, 10, 10)", color: "white", marginTop: 45}}>
                                <img style={{marginLeft: 10}} src="data:image/svg+xml;base64," width={35} height={35}>
                                </img>
                                <b style={{fontWeight: "300", marginLeft: 10}}>PHANTOM</b>
                                <div onClick={() => {setSelectionNum(1)}} className='selectionCircle' style={{width: 15, backgroundColor: selectionNum == 1 ? "rgb(40, 40, 40)" : "rgb(20, 20, 20)", height: 15, borderRadius: "50%", right: 15, position: "absolute"}}>

                                </div>
                            </div>
                            <div onClick={() => {setSelectionNum(2)}} className='rectangles2' style={{width: "80%", border: "1px solid rgba(255, 255, 255, 0.1)", display: "flex", alignItems: "center", alignContent: "center", marginLeft: "50%", transform: "translateX(-50%)", height: 50, backgroundColor: "rgb(10, 10, 10)", color: "white"}}>
                                <img style={{marginLeft: 10}} src={"data:image/svg+xml;base64,"} width={25} height={25}>
                                </img>
                                <b style={{fontWeight: "300", marginLeft: 10, display: "flex"}}>SOLFLARE</b>
                                <div onClick={() => {setSelectionNum(2)}} className='selectionCircle' style={{width: 15, backgroundColor: selectionNum == 2 ? "rgb(40, 40, 40)" : "rgb(20, 20, 20)", height: 15, borderRadius: "50%", right: 15, position: "absolute"}}>

                                </div>
                            </div>
                            <div onClick={() => {setSelectionNum(3)}} className='rectangles2' style={{width: "80%", border: "1px solid rgba(255, 255, 255, 0.1)", display: "flex", alignItems: "center", alignContent: "center", marginLeft: "50%", transform: "translateX(-50%)", height: 50, backgroundColor: "rgb(10, 10, 10)", color: "white"}}>
                                <img style={{marginLeft: 10}} src="data:image/png;base64," width={25} height={25}>
                                </img>
                                <b style={{fontWeight: "300", marginLeft: 10, display: "flex"}}>BACKPACK</b>
                                <div onClick={() => {setSelectionNum(3)}} className='selectionCircle' style={{width: 15, backgroundColor: selectionNum == 3 ? "rgb(40, 40, 40)" : "rgb(20, 20, 20)", height: 15, borderRadius: "50%", right: 15, position: "absolute"}}>

                                </div>
                            </div>
                            <div onClick={() => {setSelectionNum(4)}} className='rectangles2' style={{width: "80%", border: "1px solid rgba(255, 255, 255, 0.1)", display: "flex", alignItems: "center", alignContent: "center", marginLeft: "50%", transform: "translateX(-50%)", height: 50, backgroundColor: "rgb(10, 10, 10)", color: "white"}}>
                                <img style={{marginLeft: 10}} src="data:image/svg+xml;base64," width={25} height={25}>
                                </img>
                                <b style={{fontWeight: "300", marginLeft: 10, display: "flex"}}>TRUST</b>
                                <div onClick={() => {setSelectionNum(4)}} className='selectionCircle' style={{width: 15, backgroundColor: selectionNum == 4 ? "rgb(40, 40, 40)" : "rgb(20, 20, 20)", height: 15, borderRadius: "50%", right: 15, position: "absolute"}}>

                                </div>
                            </div>
                            <div onClick={() => {setSelectionNum(5)}} className='rectangles2' style={{width: "80%", border: "1px solid rgba(255, 255, 255, 0.1)", display: "flex", alignItems: "center", alignContent: "center", marginLeft: "50%", transform: "translateX(-50%)", height: 50, backgroundColor: "rgb(10, 10, 10)", color: "white"}}>
                                <img style={{marginLeft: 10}} src="data:image/svg+xml;base64," width={25} height={25}>
                                </img>
                                <b style={{fontWeight: "300", marginLeft: 10, display: "flex"}}>WALLETCONNECT</b>
                                <div onClick={() => {setSelectionNum(5)}} className='selectionCircle' style={{width: 15, backgroundColor: selectionNum == 5 ? "rgb(40, 40, 40)" : "rgb(20, 20, 20)", height: 15, borderRadius: "50%", right: 15, position: "absolute"}}>

                                </div>
                            </div>
                            <div style={{marginLeft: "50%", marginTop: 20, width: "80%", transform: "translateX(-50%)"}}>
                                <Button onClick={() => {setConnectWallet(false); setSelectionNum(null)}} disableRipple={true} disableFocusRipple={true} style={{width: "48%", height: 60, padding: 5, color: "white", backgroundColor: "rgba(255, 255, 255, 0.05)", border: "1px solid rgba(255, 255, 255, 0.2)"}}>CANCEL</Button>
                                <Button disableRipple={true} disableFocusRipple={true} style={{width: "48%", height: 60, padding: 5, color: selectionNum != null ? "black" : "white", backgroundColor: selectionNum != null ? "white" : "rgba(255, 255, 255, 0.05)", marginLeft: "4%", border: "1px solid rgba(255, 255, 255, 0.2)"}}>CONNECT</Button>
                            </div>
                            </Grid2>
                        </Grid2>
                        </div>
                    </div>
                    : ""
                }

            <HeaderNoFadeIn/>
            <div style={{display: "flex", justifyContent: "center"}}>
                <div style={{width: "92vw", fontSize: 60, color: "white", marginTop: 80, height: 140, fontWeight: "200", display: "flex", alignItems: "end", paddingLeft: 25}}>
                    Tokenomics
                </div>
            </div>
            <div style={{display: "flex", justifyContent: "center"}}>
                <div style={{width: "92vw", fontSize: 30, color: "white", height: 140, fontWeight: "200", display: "flex", alignItems: "center", paddingLeft: 25}}>
                    <div style={{marginLeft: 10}}>Treasuries</div>
                </div>
            </div>
            <div style={{position: "relative", fontFamily: "Geist-Mono", width: "100%", height: "max-content", minHeight: 200, display: "flex", justifyContent: "center"}}>
                <Grid2 style={{color: "white", width: "100%", display: "flex", justifyContent: "center", height: "100%"}} container>
                    <div style={{width: "100%", height: "100%", display: "flex", justifyContent: "center"}}>

                    <Grid2 style={{height: "80%", width: "92%", display: "flex", justifyContent: "center"}} size={12}>
                        <div style={{width: "95%", height: "max-content"}}>
                        <Grid2 container direction="column" style={{ height: "100%" }}>
                        <Grid2 size={12} style={{ height: "5%"}}>
                            <Grid2 container style={{ width: "100%" }}>
                            <Grid2 style={{ paddingLeft: 5, color: "darkgray", fontWeight: "300", display: "flex", justifyContent: "start"}} size={3}>
                                TREASURY
                            </Grid2>
                            <Grid2 style={{ paddingLeft: 5, color: "darkgray", fontWeight: "300", display: "flex", justifyContent: "center" }} size={3}>
                                ALLOCATION
                            </Grid2>
                            <Grid2 style={{ paddingLeft: 5, color: "darkgray", fontWeight: "300", display: "flex", justifyContent: "center" }} size={3}>
                                PERCENTAGE
                            </Grid2>
                            <Grid2 style={{ paddingLeft: 5, color: "darkgray", fontWeight: "300", display: "flex", justifyContent: "center" }} size={3}>
                                
                            </Grid2>
                            </Grid2>
                        </Grid2>

                        <Grid2 size={12} style={{ height: "95%"}}>
                            {treasuryList.map((element, index) => (
                            <Grid2
                                key={`wallet-row-${index}`}
                                container
                                style={{
                                width: "100%", // Match header width
                                height: 49,
                                fontSize: 12,
                                backgroundColor: index % 2 ? "black" : "rgb(15, 15, 15)",
                                }}
                            >
                                <Grid2 style={{ paddingLeft: 5, color: "white", fontWeight: "300", display: "flex", justifyContent: "start", alignItems: "center"}} size={3}>
                                {element.treasury}
                                </Grid2>
                                <Grid2 style={{ paddingLeft: 5, color: "white", fontSize: "0.75rem", fontWeight: "300", display: "flex", justifyContent: "center", alignItems: "center"}} size={3}>
                                <svg width={14} height={14} style={{padding: 5}} viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M6.83334 13.2219C10.4232 13.2219 13.3333 10.3117 13.3333 6.72186C13.3333 3.13201 10.4232 0.221863 6.83334 0.221863C3.24349 0.221863 0.333343 3.13201 0.333344 6.72186C0.333343 10.3117 3.24349 13.2219 6.83334 13.2219ZM6.33235 11.9049C9.00516 13.2018 12.065 11.1191 11.839 8.15694L11.5638 4.55163C11.3379 1.59058 7.99774 -0.028677 5.55099 1.63667L2.57305 3.66356C0.126292 5.32891 0.407496 9.03017 3.07927 10.3265L6.33235 11.9049Z" fill="white"></path></svg>
                                {element.allocation}
                                </Grid2>
                                <Grid2 style={{ paddingLeft: 5, color: "darkgray", fontWeight: "300", display: "flex", justifyContent: "center", alignItems: "center", fontSize: 12}} size={3}>
                                    <div style={{display: "flex", gap: 0, letterSpacing: -5, justifyContent: "center"}}>
                                        {makePercentage(element.percentage)}
                                    </div>
                                    <b style={{paddingLeft: 15, minWidth: 30}}>{element.percentage}%</b>
                                </Grid2>
                                <Grid2 style={{ paddingLeft: 5, color: "white", fontWeight: "300", display: "flex", justifyContent: "center", alignItems: "center"}} size={2.4}>
                                <div
                                    className="viewAssets"
                                    style={{ height: "100%", width: "100%", fontSize: 14, fontWeight: "400", display: "flex", alignItems: "center"}}
                                >
                                    VIEW ON SOLSCAN <span style={{fontSize: "1rem", color: "rgb(110, 110, 110)"}}>↗</span>
                                </div>
                                </Grid2>
                            </Grid2>
                            ))}
                        </Grid2>
                        </Grid2>                
                        </div>
                    </Grid2>
                    </div>
                <Grid2/>
            </Grid2>
        </div>
        <div style={{display: "flex", lineHeight: 1.5, justifyContent: "center", border: "1px solid rgba(255, 255, 255, 0.2)", marginTop: 50}}>
            <Grid2 style={{width: "100%", height: "max-content", paddingBottom: 60}} container>
                <Grid2 style={{color: "white", marginTop: 15, color: "rgba(255, 255, 255, 0.6)", display: "flex", justifyContent: "center"}} size={2}>
                    [ 01 ]
                </Grid2>
                <Grid2 style={{color: "white", display: "flex", justifyContent: "center"}} size={5}>
                    <div style={{marginTop: 10, fontSize: 25, fontWeight: "300"}}>
                        SOL 100M LPs
                    </div>
                </Grid2>
                <Grid2 style={{padding: 15, fontWeight: "200", color: "rgba(255, 255, 255, 0.6)", fontSize: 15}} size={5}>
                    The allocations are distributed among over 50 LPs, with each LP contributing over $2M to the DAO Fund smart wallet. These projects invest in us, just as we invest in them through the index.
                </Grid2>
            </Grid2>
        </div>
        <div style={{display: "flex", lineHeight: 1.5, justifyContent: "center", border: "1px solid rgba(255, 255, 255, 0.2)"}}>
            <Grid2 style={{width: "100%", height: "max-content", paddingBottom: 60}} container>
                <Grid2 style={{color: "white", marginTop: 15, color: "rgba(255, 255, 255, 0.6)", display: "flex", justifyContent: "center"}} size={2}>
                    [ 02 ]
                </Grid2>
                <Grid2 style={{color: "white", display: "flex", justifyContent: "center"}} size={5}>
                    <div style={{marginTop: 10, fontSize: 25, fontWeight: "300"}}>
                        Index Unlocked Treasury
                    </div>
                </Grid2>
                <Grid2 style={{padding: 15, fontWeight: "200", color: "rgba(255, 255, 255, 0.6)", fontSize: 15}} size={5}>
                    Are wallets that hold the OSOL coins for the purpose of contributing its value inside the main fund smart wallet to grow it, a proportion of the allocation trading decisions should be governed by community voting, and a proportion should be automated by missO and the market makers. <br/><br/> 1. 5% will be directly added to the smart wallet to sustain a high fund NAV. <br/> 2. 5% will be allocated for community-driven requests for coin purchases, based on collective voting. <br/> 3. 10% will be prepared for allocation to one of the aforementioned wallets.
                </Grid2>
            </Grid2>
        </div>
        <div style={{display: "flex", lineHeight: 1.5, justifyContent: "center", border: "1px solid rgba(255, 255, 255, 0.2)"}}>
            <Grid2 style={{width: "100%", height: "max-content", paddingBottom: 60}} container>
                <Grid2 style={{color: "white", marginTop: 15, color: "rgba(255, 255, 255, 0.6)", display: "flex", justifyContent: "center"}} size={2}>
                    [ 03 ]
                </Grid2>
                <Grid2 style={{color: "white", display: "flex", justifyContent: "center"}} size={5}>
                    <div style={{marginTop: 10, fontSize: 25, fontWeight: "300"}}>
                        OSOL Ventures
                    </div>
                </Grid2>
                <Grid2 style={{padding: 15, fontWeight: "200", color: "rgba(255, 255, 255, 0.6)", fontSize: 15}} size={5}>
                    OSOL Ventures introduces a revolutionary investment model where artificial intelligence meets community-driven venture capital. As a decentralized investment vehicle powered by MissO (the world's first AI CEO) and governed by OSOL token holders, OSOL Ventures represents a paradigm shift in how venture capital operates in the Web3 space.                </Grid2>
            </Grid2>
        </div>
        <div style={{display: "flex", lineHeight: 1.5, justifyContent: "center", border: "1px solid rgba(255, 255, 255, 0.2)"}}>
            <Grid2 style={{width: "100%", height: "max-content", paddingBottom: 60}} container>
                <Grid2 style={{color: "white", marginTop: 15, color: "rgba(255, 255, 255, 0.6)", display: "flex", justifyContent: "center"}} size={2}>
                    [ 04 ]
                </Grid2>
                <Grid2 style={{color: "white", display: "flex", justifyContent: "center"}} size={5}>
                    <div style={{marginTop: 10, fontSize: 25, fontWeight: "300"}}>
                        Ecosystem & OBOT Treasury
                    </div>
                </Grid2>
                <Grid2 style={{padding: 15, fontWeight: "200", color: "rgba(255, 255, 255, 0.6)", fontSize: 15, display: "flex", justifyContent: "center"}} size={5}>
                    <div style={{padding :10}}>
                        <b style={{color: "white"}}>OBOT Holders Strategic Reserve: 15%</b>
                        <br/>
                        • Rewards for existing OBOT holders
                        • Ecosystem cross-pollination incentives
                        • Long-term holder benefits

                        <br/>
                        <br/>
                        <br/>

                        <b style={{color: "white"}}>Ecosystem Development: 10%</b>
                        <br/>
                        • R&D initiatives
                        • Product development funding
                        • Technical infrastructure improvements

                        <br/>
                        <br/>
                        <br/>

                        <b style={{color: "white"}}>Community Rewards: 15%</b>
                        <br/>
                        • Airdrops for active participants
                        • User acquisition incentives
                        • Early adopter rewards

                        <br/>
                        <br/>
                        <br/>

                        Exchange Listings: 15%
                        • Major exchange listing fees
                        • Market making provisions
                        • Trading pair establishment

                    </div>
                                        
                    <div style={{padding :10}}>
                        <b style={{color: "white"}}>Growth & Marketing (35%)</b>
                        <br/>
                        • Key Opinion Leader (KOL) collaborations
                        • Industry influencer engagements
                        • Strategic advisor allocations

                        <br/>
                        <br/>
                        <br/>

                        <b style={{color: "white"}}>Marketing Operations: 15%</b>
                        <br/>
                        • Global marketing campaigns
                        • Brand awareness initiatives
                        • Community building activities

                        <br/>
                        <br/>
                        <br/>

                        <b style={{color: "white"}}>Events & Education: 10%</b>
                        <br/>
                        • Hackathons
                        • Developer workshops
                        • Community meetups
                        • Educational content creatio

                        <br/>
                        <br/>
                        <br/>


                        Liquidity Reserves: 10%
                        • DEX liquidity provision
                        • Market stability fund
                        • Emergency reserves

                    </div>


                </Grid2>
                
            </Grid2>
        </div>
        <div style={{display: "flex", lineHeight: 1.5, justifyContent: "center", border: "1px solid rgba(255, 255, 255, 0.2)"}}>
            <Grid2 style={{width: "100%", height: "max-content", paddingBottom: 60}} container>
                <Grid2 style={{color: "white", marginTop: 15, color: "rgba(255, 255, 255, 0.6)", display: "flex", justifyContent: "center"}} size={2}>
                    [ 05 ]
                </Grid2>
                <Grid2 style={{color: "white", display: "flex", justifyContent: "center"}} size={4}>
                    <div style={{marginTop: 10, fontSize: 25, fontWeight: "300"}}>
                        OSOL CHARITY

                    </div>
                </Grid2>
                <Grid2 style={{padding: 15, fontWeight: "200", color: "rgba(255, 255, 255, 0.6)", fontSize: 15}} size={6}>
                    OSOL introduces OSOL CHARITY, a revolutionary approach to charitable giving where our community and AI work together to create meaningful social impact. 3% of all OSOL fund management fees are automatically allocated to charitable causes, with recipients chosen through community voting and vetted by MissO AI.
                    
                    <div style={{width: "100%", display: "flex", justifyContent: "center"}}>

                        <div style={{padding: 10}}>
                            <b style={{color: "white"}}>Program Structure</b>
                            
                            <br/>

                            Key Features<br/><br/>
                            • 100% of allocated funds go to chosen charities<br/>
                            • Smart contract automated distributions<br/>
                            • Public voting records on chain<br/>
                            • Regular impact assessment updates<br/><br/>
                            Example Initiatives<br/>
                            • Funding coding bootcamps in underserved communities<br/>
                            • Supporting AI education programs<br/>
                            • Backing environmental tech solutions<br/>
                            • Enabling digital literacy initiatives<br/>
                         </div>
                         <div style={{padding: 10}}>
                            <b style={{color: "white"}}>Program Structure</b>
                            
                            <br/>
                            By combining community wisdom with AI verification, OSOL GIVES creates a new model for crypto philanthropy that is:
                            <br/>
                            • Transparent<br/>
                            • Community-driven<br/>
                            • Impact-focused<br/>
                            • Globally accessible<br/>

                         </div>


                    </div>

                </Grid2>
                
            </Grid2>
        </div>
        <Footer setModalActivation={setConnectWallet} modalActivation={connectWallet}/>
        </>
    )
}